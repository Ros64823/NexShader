...
vec3 waterNormal(vec3 n, vec2 uv){float w=sin((uv.x+frameTimeCounter*0.025)*80.0)+cos((uv.y-frameTimeCounter*0.018)*64.0); return safeNormalize(n+vec3(w*0.035*float(WATER_QUALITY),0.0,w*0.025*float(WATER_QUALITY)),vec3(0.0,1.0,0.0));}
#endif
