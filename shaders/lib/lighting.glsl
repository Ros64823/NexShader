...
vec3 applyLighting(vec3 albedo, vec3 normal, vec3 viewPos, float shadow){vec3 l=safeNormalize(sunPosition,vec3(0.0,1.0,0.0)); float ndl=saturate(dot(normal,l)); float wrap=saturate((dot(normal,l)+0.35)/1.35); vec3 amb=ambientColor()*(0.55+0.45*normal.y); vec3 sun=sunColor()*(ndl*shadow+0.18*wrap); return albedo*(amb+sun);}
#endif
